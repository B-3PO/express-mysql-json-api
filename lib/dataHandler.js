...
exports.get = function (req, res, config) {
  ...
  runQuery(buildQuery(structure, req.params.id), function (status, data) {
    // TODO handle no data
    res.status(status).send(formatData(structure, data, req.params.id === undefined));
  });
};



exports.addEdit = function (req, res, config) {
  ...
  runMultipleQueryies(buildUpdateQuery(data, meta, id), function (status) {
...
function formatData(structure, data, toMany) {
  ...
  returnData.data = dataByType[structure.type] !== undefined ? dataByType[structure.type] : toMany === true ? [] : {};
...
function formatDataAsJSONAPI(data, structure) {
  ...
  var key = keys.pop();
  var flattendStructure = getFlattendStructure(structure);
  ...
  while (key !== undefined) {
    ...
    while (i < length) {
      data[key][i] = {
        id: data[key][i].uuid,
        type: key,
        attributes: getReturnAttrubutes(flattendStructure[key], data[key][i]),
...
function getFlattendStructure(structure, obj) {
  obj = obj || {};
  ...
  if (structure.relationships !== undefined) {
    ...
    while (key !== undefined) {
      getFlattendStructure(structure.relationships[key], obj);
      key = keys.pop();
    }
  }
...
// this is meant for updating relation along side of adding
function runMultipleQueryies(queries, callback) {
...
function buildRelationshipQuery(data, parentId, property, resource) {
  ...
  if (relationship.parentRelation.manyToMany === true) {
    ...
    query = 'update ' + relationship.parentRelation.parentTable + ',' + type.table + '\n';
    query += 'set ' + type.table + '.' + relationship.parentRelation.parentField + '=' + relationship.parentRelation.parentTable + '.id\n';
    query += 'where ' + relationship.parentRelation.parentTable + '.' + getIdField(parentId) + '=\'' + parentId + '\' and ' + type.table + '.' + getIdField(data.id) + '=\'' + data.id + '\'';
...
function buildUpdateQuery(data, meta, id) {
  var type = types[data.type];
  var convertedAttrs = convertAttributesForSQL(data.attributes, type.attributes);
  var addConstraint = type.constrain === true && meta && meta.constraint ? true : false;
  var oneToMany = false;

  if (addConstraint) {
    buildStructure(resources[meta.constraint.resource]);
    var contraintResouce = resources[meta.constraint.resource];
    var constraintRelation = getRelationshipFromParentType(contraintResouce.type, data.type);
    if (constraintRelation.oneToMany === true) {
      oneToMany = true;
    }
  }
  ...
  if (oneToMany) {
    query += ',' + constraintRelation.parentField;
  }
  ...
  if (oneToMany) {
    ...
    query += ',' + constraintRelation.parentTable + '.id\n';
    query += 'from ' + constraintRelation.parentTable;
    query += '\nwhere ' + constraintRelation.parentTable + '.' +getIdField(meta.constraint.id) + '=\'' + meta.constraint.id + '\'';
  } else {
    query += 'values (\'' + id + '\'';
    query += Object.keys(convertedAttrs).reduce(function (a, key) {
      if (convertedAttrs[key] !== undefined) {
  ...
  if (addConstraint === true && oneToMany === false) {
    query += ';\n' + buildRelationshipQuery(data, meta.constraint.id, constraintRelation.field, contraintResouce);
  }
  return query;
}


function getRelationshipFromParentType(parentType, childTypeName) {
  var i = 0;
  var length = parentType.relationships.length;

  while (i < length) {
    if (parentType.relationships[i].type === childTypeName) {
...
function addJoins(relations, queryObj) {
  ...
  while (key !== undefined) {
    ...
    } else if (parentRelation.oneToMany === true) {
      queryObj.joins.push('left join ' + relationship.table + ' on ' + parentRelation.parentTable + '.id' + ' = ' + relationship.table + '.' + parentRelation.parentField);
...
function getClientStructure(structure) {
  ...
  if (structure.relationships !== undefined) {
    clientStructure.relationships = getClientStructureRelationships(structure.relationships, structure);
  }

  return clientStructure
}

function getClientStructureRelationships(relationships, parentResource) {
  ...
  while (key !== undefined) {
    ...
    if (typeObj.constrain === true) {
      relObj[key].meta = relObj[key].meta || {};
      relObj[key].meta.constraint = {
        resource: parentResource.resouceName
      };
    }

    if (relation.parentRelation !== undefined) {
      relObj[key].meta = relObj[key].meta || {};
      relObj[key].meta.toMany = relation.parentRelation.manyToMany || relation.parentRelation.oneToMany || false;
    }

    if (relation.relationships !== undefined) {
      relObj[key].relationships = getClientStructureRelationships(relation.relationships, relation);
    }
...
function buildStructure(config) {
  ...
  if (config.built === true || typeof config.type === 'object') { return config; } // structure was built succesfully
