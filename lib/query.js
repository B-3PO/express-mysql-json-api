...
exports.get = function (structure, id, callback) {
  ...
  addJoins(structure.relationships, queryBuilder);
...
function addJoins(relationships, queryBuilder) {
  ...
  while (key !== undefined) {
    queryBuilder.addRelation(relationships[key]);
    addJoins(relationships[key].resource.relationships, queryBuilder);
    key = keys.pop();
  }
}
...
function formatJsonApi(dataByType, structure, toMany) {
  ...
  returnData.data = toMany ? data : data[1];
